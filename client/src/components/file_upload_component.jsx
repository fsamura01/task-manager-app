...
function FileUploadComponent({
  ...
  const fetchExistingFiles = async () => {
    try {
      ...
      const normalizedFiles = files.data.map((file) => ({
        ...
        downloadUrl: file.download_url || `${API_BASE_URL}/files/${file.id}/download`,
        metadata: {
          projectId: file.project_id,
          taskId: file.task_id,
          taskTitle: tasks.find((t) => t.id === file.task_id)?.title || "Unknown Task",
  ...
  const handleDeleteFile = async (fileId) => {
    try {
      ...
      setCompletedUploads((prev) => prev.filter((file) => file.serverResponse?.id !== fileId)
      );
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.upload.addEventListener("progress", (event) => {
        if (event.lengthComputable) {
          ...
          setUploadingFiles((prev) => prev.map((file) => file.id === fileData.id
  ...
  const handleUpload = async () => {
    ...
    const tasksToAssociate = allowTaskSelection && selectedTaskIds.length > 0
      ? selectedTaskIds
    ...
    try {
      ...
      failed.forEach(({ fileData, error, taskIdToAssociate }) => {
        ...
        setErrors((prev) => [
          ...prev,
          `Failed to upload "${fileData?.name || "unknown file"}"${taskInfo}: ${error?.message || "Unknown error"}`,
        ]);
      });
        ...
        <Col>
          <Card className="mb-4">
            ...
            <Card.Body>
              ...
              <div
                className={`border-2 border-dashed rounded p-4 text-center mb-3 ${isDragOver
